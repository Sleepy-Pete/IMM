...
#define TBE_SMALL_NUMBER 1.e-8f
   ...
   class TBVector
   {
    ...
    public:
      ...
      /// Returns the squared magnitude of this vector. Useful for comparing magnitudes without using expensive sqrt
      /// function.
      ...
      /// Access the x,y,z values of the vector like an array. vector[0] is x, vector[1] is y, vector[2] is z
      /// \param index vector[0] is x, vector[1] is y, vector[2] is z
      ...
      inline static void rotateByVectors(const TBVector &forwardVector, const TBVector &upVector,
      ...
      /// \param matrixOut The output matrix. Needs to be initialised elsewhere as an array of floats at least 9 long.
      inline static void getMatrixFromVectors(TBVector forwardVector, TBVector upVector, float *matrixOut);
      ...
      /// \param matrixOut The output matrix. Needs to be initialised elsewhere as an array of floats at least 9 long.
      inline static void getFromToRotationMatrix(TBVector fromPoint, TBVector toPoint, float *matrixOut);

      /// Get the azimuth/elevation/distance of a vector (conversion from Cartesian to gaming type spherical
      /// coordinates)
   ...
   {
      return TBVector(vectorA.y * vectorB.z - vectorA.z * vectorB.y, vectorA.z * vectorB.x - vectorA.x * vectorB.z,
   ...
   inline float TBVector::Angle(const TBVector &vectorA, const TBVector &vectorB)
   {
      ...
      float normDot = TBVector::DotProduct(vectorA, vectorB) / denom;
   ...
   inline void TBVector::rotateByVectors(const TBVector &forwardVector, const TBVector &upVector,
                                         TBVector &rotatedVectorOut)
   {
      float rotMatrix[3][3] = {{0.f, 0.f, 0.f}, {0.f, 0.f, 0.f}, {0.f, 0.f, 0.f}};
      TBVector N1(forwardVector.x, forwardVector.y, forwardVector.z), V1(upVector.x, upVector.y, upVector.z);
   ...
   inline void TBVector::getMatrixFromVectors(TBVector forwardVector, TBVector upVector, float *matrixOut)
   {
   ...
   inline void TBVector::getFromToRotationMatrix(TBVector fromPoint, TBVector toPoint, float *matrixOut)
   {
      // swap y and z:
      float fr[3] = {fromPoint.x, fromPoint.z, fromPoint.y};
      float to[3] = {toPoint.x, toPoint.z, toPoint.y};

      float u[3] = {fr[1] * to[2] - fr[2] * to[1], fr[2] * to[0] - fr[0] * to[2], fr[0] * to[1] - fr[1] * to[0]};

      if (std::abs(u[0]) < TBE_SMALL_NUMBER && std::abs(u[1]) < TBE_SMALL_NUMBER && std::abs(u[2]) < TBE_SMALL_NUMBER)
      {
      ...
      // see https://en.wikipedia.org/wiki/Rotation_matrix#Rotation_matrix_from_axis_and_angle for details
   ...
   inline Aed TBVector::getAedFromVector(TBVector vector)
   {
      ...
      result.elevation = std::atan2(vector.y, std::sqrt(vector.x * vector.x + vector.z * vector.z)) * 180.f / M_PIF;
   ...
   inline TBVector TBVector::getVectorFromAziEle(float azimuth, float elevation)
   {
      ...
      return TBVector(std::sin(azi_r) * cos_ele, std::sin(ele_r), std::cos(azi_r) * cos_ele);
   }
