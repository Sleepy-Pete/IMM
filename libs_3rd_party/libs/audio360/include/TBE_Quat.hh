...
#define TBE_SMALL_NUMBER 1.e-8f
   ...
   /// Unless specifically stated otherwise, the quaternion operations in this class assume a left-handed Unity-style
   /// coordinate system where x=right, y=up and z=forward. You can use the convertFromOpenGLQuaternion(.) function to
   /// convert quaternions from an OpenGL-style coordinate system.
   class TBQuat
   {
    public:
      float x;
      ...
      /// Calculate azimuth, elevation and distance of a source respective to the listener's position and orientation
      ...
      inline static Aed getAedFromQuat(TBQuat listenerQuat, TBVector listenerPosition, TBVector sourcePosition);
      ...
      /// \return The rotation as a quaternion
      inline static TBQuat getQuatFromMatrix(float m00, float m01, float m02, float m10, float m11, float m12,
   ...
   TBQuat::TBQuat(float xValue, float yValue, float zValue, float wValue) : x(xValue), y(yValue), z(zValue), w(wValue)
   {
   }
   ...
   inline TBQuat TBQuat::operator*(const TBQuat &other) const
   {
      ...
      float n = std::sqrt(result.x * result.x + result.y * result.y + result.z * result.z + result.w * result.w);
      result.x /= n;
      result.y /= n;
      result.z /= n;
      result.w /= n;
   ...
   inline std::string TBQuat::toString(int precision) const
   {
      ...
      stream << std::fixed << std::setprecision(precision) << "[" << x << ", " << y << ", " << z << ", " << w << "]";
      return stream.str();
   }
   ...
   inline TBQuat TBQuat::getQuatFromEulerAngles(float xRotation, float yRotation, float zRotation)
   {
      ...
      // this calculation may output quaternions which have ALL components negative as compared to what Unity might
      // calculate,
      // BUT, this result is still totally correct because they actually represent the same rotation: q = -q
   ...
   inline Aed TBQuat::getAedFromQuat(TBQuat listenerQuat, TBVector sourcePosition, TBVector listenerPosition)
   {
      TBVector relativePos(sourcePosition.x - listenerPosition.x, sourcePosition.y - listenerPosition.y,
   ...
   inline TBVector TBQuat::antiRotateVectorByQuat(TBQuat rotQuat, TBVector vector)
   {
      ...
      // multiply to calculate final quat after rotation
      TBQuat firstProd = quatProductUnNormalised(rotQuat_conj, TBQuat(vector.x, vector.y, vector.z, 0.0f));
      TBQuat outQuat = quatProductUnNormalised(firstProd, rotQuat);

      return TBVector(outQuat.x, outQuat.y, outQuat.z);
   }

   // get the quaternion rotation from one vector to another vector (same output as Unity's SetFromToRotation function).
   inline TBQuat TBQuat::getFromToQuatRotation(TBVector fromVec, TBVector toVec)
   {
      ...
      float arg =
          std::abs(0.5f * (1.f - normDot)); // the 'abs' is not necessary in theory, but in practise is needed due
                                            // to precision errors, otherwise we might feed negative numbers into
                                            // the square root!
   ...
   {
      return TBVector(2 * (q.x * q.z + q.w * q.y), 2 * (q.y * q.z - q.w * q.x), 1 - 2 * (q.x * q.x + q.y * q.y));
   }

   inline TBVector TBQuat::getUpFromQuat(TBQuat q)
   {
      return TBVector(2 * (q.x * q.y - q.w * q.z), 1 - 2 * (q.x * q.x + q.z * q.z), 2 * (q.y * q.z + q.w * q.x));
   }

   inline TBVector TBQuat::getRightFromQuat(TBQuat q)
   {
      return TBVector(1 - 2 * (q.y * q.y + q.z * q.z), 2 * (q.x * q.y + q.w * q.z), 2 * (q.x * q.z - q.w * q.y));
   }
   ...
   inline TBQuat TBQuat::getQuatFromForwardAndUpVectors(TBVector forwardVector, TBVector upVector)
   {
      ...
      return TBQuat::getQuatFromMatrix(right.x, upVector.x, forwardVector.x, right.y, upVector.y, forwardVector.y,
                                       right.z, upVector.z, forwardVector.z);
   }
   ...
   inline TBQuat TBQuat::getQuatFromMatrix(float m00, float m01, float m02, float m10, float m11, float m12, float m20,
                                           float m21, float m22)
   {
   ...
   {
      auto random_num = []() { return 2.f * static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) - 1.f; };
