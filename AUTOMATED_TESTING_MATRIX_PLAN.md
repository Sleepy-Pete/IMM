...
| Standalone Windows VR | Packaged legacy OpenGL VR settings exist; Windows OpenGL VR smoke lane requires HMD initialization and frame-submission markers; Windows OpenXR standalone VR smoke lane exists with session/stereo/frame contract markers | Need passing headset-run artifacts before release evidence can be considered complete |
